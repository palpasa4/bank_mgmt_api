...
from modules.user.exceptions import UserPermissionDeniedException
...
def check_ifadmin(id:str,db):
    if check_role(id, db) != "admin":
        ...
        raise UserPermissionDeniedException(message="User not allowed.",status_code=401)
...
def check_user_details(model:CreateUserModel):
    ...
    if len(model.username)>20:
        ...
        raise UsernameTooLongException(message="Username cannot include more than 20 characters!",status_code=400)

    if model.opening_balance < 500:
        # business rule violation
        logger.error("Bank account creation failed: opening balance below minimum requirement")
        raise OpeningBalanceException(message="Minimum opening balance is 500!",status_code=400)
